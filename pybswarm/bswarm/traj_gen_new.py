...
class Trajectory:
    ...
    def __init__(self, c, T, n):
        ...
        self.T = T
        self.n = n
        self.S = np.hstack([[0], np.cumsum(T)])
    ...
    def compute_trajectory(self):
        ...
        for i in range(self.n_legs):
            ...
            beta = ti - S[i]
            cp = c[n * i:n * (i + 1)]
            cv = poly_derivative(cp)
            ca = poly_derivative(cv)
            cj = poly_derivative(ca)
            cs = poly_derivative(cj)
...
def trajectory_coeff(beta, m, n):
    """
    @param beta: scaled time (t - Si)
...
def plan_min_accel(x_list, T):
    ...
    for i in range(n_legs):
        # p1(0) = x[0] setting coordinates of waypoints
...
def plan_min_snap(x_list, T):
    ...
    @param T: duration list, length one less than x/v list
    """
    n = 8  # this will be fixed to support 7th order polynomials
    if not len(x_list) == len(T) + 1:
        raise ValueError('x_list must be 1 longer than T')
    ...
    S = np.hstack([[0], np.cumsum(T)]) # time at each waypoint

    eq_num = 0
    for i in range(n_legs):

        # conditions : set waypoints
        for m in [0, 1, 2]:
            # p0^(m)(0) = 0 # start of the leg
            A[eq_num, n * i:n * (i + 1)] = trajectory_coeff(
                beta=0, m=m, n=n)
            if m == 0: # pos
                b[eq_num] = x_list[i]
        ...
        # initial conditions
        if i == 0:
            # p(0)^4(0) = 0 : snap at the first leg is zero
                A[eq_num, n * i:n * (i + 1)] = trajectory_coeff(
                    beta=0, m=4, n=n)
                b[eq_num] = 0
                eq_num += 1
        ...
        if i == n_legs - 1:
            for m in [0, 1, 2, 3]:
                # p(n-1)^m(1) = 0 : last leg, v, a, j are zero
                A[eq_num, n * i:n * (i + 1)] = trajectory_coeff(
                    beta=S[i+1]-S[i], m=m, n=n)
                if m == 0:
                    b[eq_num] = x_list[i + 1]
